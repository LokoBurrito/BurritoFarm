...
MS.Parent = M
USER.Text = "| Username - @LokoBurrito"
...
if (_G.AutofarmSettings.Credits ~= "burrito autofarm") then 
...
local Log = function(Msg)
	print(os.date("%X").." | burrito's stupid farm - "..Msg)
end
...
local Attack = function()
	...
	if (Mode == nil) then
		return Log("NOT AN ATTACK METHOD!!!")
	end
	...
	elseif (Mode == 3) then
		if (Player.DataFolder.Currency.Value < 200) then 
			task.spawn(function()
				EMPTY.Text = "| u r too broke 4 knife"
				task.wait(10)
				EMPTY.Text = "|"
			end)
		...
		task.spawn(function()
			if (os.time() >= LastLog) then 
				local s, e = pcall(function()
					SendLog(_G.AutofarmSettings.Webhook, {Player.Name, Player.UserId, WALLET.Text, PROFIT.Text, TIMER.Text, BROKEN.Text, "| burrito autofarm"})
				end)
				if (e) then 
